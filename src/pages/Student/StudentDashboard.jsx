...
import { Bell, BookOpenCheck, CheckCircle2, FolderGit2, HardDriveUpload, LogOut, Play, ShieldCheck, Sparkles, Target, Video, X } from "lucide-react";
...
export default function StudentDashboard() {
  ...
  const renderCourse = (course) => {
    ...
    return (
    <div key={courseId} className={`overflow-hidden rounded-2xl border bg-white shadow-[0_14px_40px_-30px_rgba(15,23,42,0.28)] transition hover:-translate-y-0.5 hover:shadow-[0_20px_48px_-30px_rgba(15,23,42,0.24)] ${isSelected ? "border-[#1768df] ring-2 ring-[#1768df]/15" : "border-slate-200"}`}>
      <div className="grid sm:grid-cols-[9.5rem_minmax(0,1fr)]">
        <div className="relative flex min-h-36 flex-col justify-between overflow-hidden bg-[radial-gradient(circle_at_78%_22%,rgba(76,222,153,0.68),transparent_30%),linear-gradient(145deg,#082e68,#063b8e_55%,#04265f)] p-4 text-white">
        ...
        <div className="p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.17em] text-[#1768df]">Online course</p>
          ...
          <div className="mt-4 flex items-center justify-between gap-3 border-t border-slate-100 pt-4">
            ...
            <button
              ...
              className={`inline-flex shrink-0 items-center gap-2 rounded-xl px-3.5 py-2 text-sm font-semibold transition ${isSelected ? "bg-[#1768df] text-white" : "bg-[#edf4ff] text-[#1768df] hover:bg-[#1768df] hover:text-white"}`}
            >
  ...
  return (
    <div className="min-h-screen bg-[#f5f8fc] text-cert-ink">
      ...
      <nav className="hidden lg:fixed lg:inset-y-0 lg:z-30 lg:flex lg:w-60 lg:flex-col lg:border-r lg:border-white/10 lg:bg-[linear-gradient(180deg,#063b8e_0%,#04265f_100%)] lg:p-5 lg:text-white" aria-label="Student workspace navigation">
        ...
        <div className="mt-3 flex flex-1 flex-col gap-2">
          <button type="button" onClick={() => openPanel("courses")} className={`inline-flex items-center gap-2 rounded-xl px-3 py-3 text-sm font-semibold transition ${activePanel === "courses" ? "bg-[#1768df] text-white shadow-lg shadow-blue-950/25" : "text-white/85 hover:bg-white/10"}`}>
            <span>Courses</span><span className="rounded-full bg-white px-2 py-0.5 text-cert-green-dark">{courses.length}</span>
          </button>
          <button type="button" onClick={() => openTaskPage("assignment")} className={`inline-flex items-center gap-2 rounded-xl px-3 py-3 text-sm font-semibold transition ${activePanel === "assignments" ? "bg-[#1768df] text-white shadow-lg shadow-blue-950/25" : "text-white/85 hover:bg-white/10"}`}>
            <span>Assignments</span><span className="rounded-full bg-white px-2 py-0.5 text-cert-green-dark">{assignmentTasks.length}</span>
          </button>
          <button type="button" onClick={() => openTaskPage("project")} className={`inline-flex items-center gap-2 rounded-xl px-3 py-3 text-sm font-semibold transition ${activePanel === "projects" ? "bg-[#1768df] text-white shadow-lg shadow-blue-950/25" : "text-white/85 hover:bg-white/10"}`}>
          ...
          {[
            ...
            <button key={label} type="button" onClick={() => openPanel("task-status", { status })} className={`inline-flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${activePanel === "task-status" && taskStatusFilter === status ? "bg-[#1768df] text-white shadow-lg shadow-blue-950/25" : "text-white/75 hover:bg-white/10"}`}>
              <span>{label}</span>
              <span className="rounded-full bg-white px-2 py-0.5 text-cert-green-dark">{value}</span>
            </button>
          ))}
          <button type="button" onClick={() => openPanel("certificate")} className={`inline-flex items-center gap-2 rounded-xl px-3 py-3 text-sm font-semibold transition ${activePanel === "certificate" ? "bg-[#1768df] text-white shadow-lg shadow-blue-950/25" : "text-white/85 hover:bg-white/10"}`}>
      ...
      <header className="flex items-center justify-end gap-4 rounded-2xl border border-slate-100 bg-white px-5 py-4 shadow-[0_12px_30px_-24px_rgba(15,23,42,0.24)]">
        <div className="flex items-center gap-3"><Bell size={19} className="hidden text-slate-500 sm:block" aria-hidden="true" /><span className="hidden text-sm font-medium text-slate-600 sm:block">{profile.full_name || "Student"}</span><span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#e6efff] font-semibold text-[#1768df]">{(profile.full_name || "S").charAt(0).toUpperCase()}</span></div>
      </header>
      <section className="grid gap-5 rounded-2xl border border-slate-100 bg-white p-6 shadow-[0_16px_38px_-30px_rgba(15,23,42,0.3)] xl:grid-cols-[minmax(0,1fr)_15rem] xl:items-center">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#1768df]">Student dashboard</p>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-2xl bg-[#eef4ff] p-3 text-center"><p className="text-2xl font-bold text-[#1768df]">{courses.length}</p><p className="mt-1 text-[0.65rem] font-semibold text-slate-500">COURSES</p></div>
