...
begin
  ...
  for question in select value from jsonb_array_elements(question_set) loop
    ...
    if coalesce(p_answers ->> (question ->> 'id'), '') = coalesce(question ->> 'correct_option', '') then correct_answers := correct_answers + 1; end if;
  end loop;
  ...
  return jsonb_build_object('score', result_score, 'passed', did_pass, 'attempt_number', prior_attempts + 1, 'attempts_remaining', 3 - (prior_attempts + 1), 'next_attempt_at', case when did_pass then null else now() + interval '1 day' end);
end;
$$;
