-- A passing final-test result issues one certificate immediately. The client
-- then sends the certificate email using the returned certificate ID.
...
begin
  ...
  if did_pass then
    ...
    if issued_certificate_id is null then
      select id into issued_certificate_id
      from public.certificates
      where student_id = (select auth.uid()) and course_id = test_record.course_id
      limit 1;
    end if;
  end if;
